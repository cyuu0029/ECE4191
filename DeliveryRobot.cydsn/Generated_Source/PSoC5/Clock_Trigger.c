...
void Clock_Trigger_Start(void) 
{
    ...
    Clock_Trigger_CLKEN |= Clock_Trigger_CLKEN_MASK;
	Clock_Trigger_CLKSTBY |= Clock_Trigger_CLKSTBY_MASK;
}
...
void Clock_Trigger_Stop(void) 
{
    ...
    Clock_Trigger_CLKEN &= (uint8)(~Clock_Trigger_CLKEN_MASK);
	Clock_Trigger_CLKSTBY &= (uint8)(~Clock_Trigger_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Trigger_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_StopBlock(void) 
{
    if ((Clock_Trigger_CLKEN & Clock_Trigger_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_Trigger__CFG3)
        CLK_DIST_AMASK = Clock_Trigger_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Trigger_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Trigger__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Trigger_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Trigger_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Trigger_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_Trigger_CLKSTBY &= (uint8)(~Clock_Trigger_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_Trigger_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_Trigger_GetSourceRegister();
    uint16 oldDivider = Clock_Trigger_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Trigger_CLKEN & Clock_Trigger_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Trigger_DIV_PTR, clkDivider);
                Clock_Trigger_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Trigger_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Trigger_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Trigger_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clock_Trigger_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Trigger_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_SetModeRegister(uint8 modeBitMask) 
{
    Clock_Trigger_MOD_SRC |= modeBitMask & (uint8)Clock_Trigger_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Trigger_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_Trigger_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Trigger_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Trigger_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Trigger_GetModeRegister(void) 
{
    return Clock_Trigger_MOD_SRC & (uint8)(Clock_Trigger_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Trigger_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_Trigger_GetDividerRegister();
    uint8 oldSrc = Clock_Trigger_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Trigger_MOD_SRC |= CYCLK_SSS;
        Clock_Trigger_MOD_SRC =
            (Clock_Trigger_MOD_SRC & (uint8)(~Clock_Trigger_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Trigger_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Trigger_GetSourceRegister(void) 
{
    return Clock_Trigger_MOD_SRC & Clock_Trigger_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Trigger_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_Trigger_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_Trigger_PHASE = clkPhase & Clock_Trigger_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Trigger_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_Trigger_GetPhaseRegister(void) 
{
    return Clock_Trigger_PHASE & Clock_Trigger_PHASE_MASK;
}
